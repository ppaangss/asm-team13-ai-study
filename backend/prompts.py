...
위험도 판단 기준 (반드시 아래 기준만 사용):
- 상 (위험 점수 67~100): 답변에서 구체적 수치·근거·계획이 전혀 없었거나, 핵심 질문을 회피·모른다고 했을 때
- 중 (위험 점수 34~66): 답변에 방향은 있었지만 근거나 수치가 불충분하거나 추상적으로 마무리됐을 때
- 하 (위험 점수 0~33): 답변에 구체적 수치·계획·사례가 제시되어 납득 가능했을 때 (단, 허점이 존재하므로 리포트에는 포함)

위험 점수(0~100): 위험도와 반드시 일치해야 함 (상=67~100, 중=34~66, 하=0~33)
...
| 섹션 | 핵심 허점 | 위험도 | 위험 점수 | 보완 제안 |
|------|-----------|--------|-----------|-----------|
| (섹션명) | (허점 설명) | 상/중/하 | 0~100 | (구체적 방향) |

위험도와 위험 점수는 반드시 위 기준에 따라 대화 내용 근거로 판단할 것. 임의로 배분하지 말 것.
